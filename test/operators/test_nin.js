              ...
              (function(id) {
                ...
                db.users.put(id, user, function(err) {
                  ...
                  if (users_in == USER_COUNT) {
                    ...
                    db.users.find({'age' : {$nin: [32, 42, 30]}}) (function(err, key, value) {
                      if (err) { next(err); return; }
                      assert.deepEqual(value, USERS[key]);
                      assert.ok(value.age == 28 || value.age == 29 || value.age == 35, 'age is in [32, 42, 30] for found user with key ' + key + ' and age ' + value.age);
                      users_found ++;
                      assert.ok(users_found <= 4, 'already found ' + users_found + ' users');
                      if (users_found == 4) {
