  ...
  if (this.queue.length > 0 && pos >= this.queue_starts_at_pos) {
    ...
    while(queue_pos < this.queue.length && read_length < length) {
      ...
      if (pos + length >= current_segment.p && current_segment.p + current_segment.l >= pos) {
        ...
        ends_at = length - read_length;
        if (ends_at > current_segment.l) {
          ends_at = current_segment.l;
        }
        var buffer = new Buffer(current_segment.s, 'utf8');
        if (starts_at >= 0 && ends_at > 0) {
          buffer.copy(return_buffer, read_length, starts_at, ends_at);
          read_length += (ends_at - starts_at);
        }
      }
      queue_pos ++;
    }
    retbuf = new Buffer(read_length);
    return_buffer.copy(retbuf, 0, 0, read_length)
    callback(null, retbuf, read_length);
  } else {
