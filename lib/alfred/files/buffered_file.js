...
BufferedFile.prototype.flushSync = function() {
  while (this.queue.length > 0) {
    ...
    if (written != segment.l) {
      throw new Error('file.rawWrite_sync returned written length (' + written + ') different from segment length (' + segment.l + ')');
    }
  }
};
...
BufferedFile.prototype.fetch = function(pos, length, callback) {
  if (pos >= this.queue_starts_at_pos) {
    ...
    if (segment) {
      if (segment.l != length) {
        callback(new Error('segment was found in position ' + pos + ', but length is ' + segment.l + ', not ' + length + '. content: ' + segment.s));
        return;
      } else {
...
BufferedFile.prototype.tryRead = function(pos, length, callback) {
  var self = this;
  if (this.queue.length > 0 && pos >= this.queue_starts_at_pos) {
    var return_buffer = new Buffer(length);
    var read_length = 0;
    var queue_pos = 0;
    var current_segment;
    var found = false;
    var source_start, source_end, target_start, target_end, ret_buffer, copy_length;
    var started_at;
    
    while(queue_pos < this.queue.length && read_length < length) {
      current_segment = this.queue[queue_pos];
      //if (current_segment.p == pos || current_segment.p + current_segment.l >= pos && current_segment.p <= pos + length ) {
        // this segment has something we want
      var offset = current_segment.p - pos;
      var buf = new Buffer(current_segment.s);
      
      source_start = offset < 0 ? - offset : 0;
      source_end = Math.max(current_segment.l + (offset < 0 ? offset : 0), current_segment.l);
      copy_length = source_end - source_start;
      
      target_start = offset < 0 ? 0 : offset;
      target_end = target_start + copy_length;
      if (source_end > source_start) {
        if (started_at === undefined) {
          if (target_start !== 0) {
            callback(new Error('copy starts at target position ' + target_start));
            return;
          }
          started_at = 0;
        }
        if (copy_length + target_start > length) {
          source_end -= (copy_length + target_start) - length;
          copy_length -= (copy_length + target_start) - length;
        }
        buf.copy(return_buffer, target_start, source_start, source_end);
    ...
    if (!found) {
      ...
      callback(null, return_buffer.slice(0, read_length), read_length);
    }
  } else {
    File.prototype.tryRead.call(this, pos, length, callback);
  }
};
