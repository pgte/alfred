...
OrderedIndex.prototype.rangeSync = function(start, end, exclusive_start) {
  var keys = [];
  var gotten = this.bplustree.rangeSync(start, end, exclusive_start);
...
OrderedIndex.prototype.filter = function(filter_function, callback, null_on_not_found){
  ...
  this.bplustree.each(function(key, values) {
    values.forEach(function(value) {
      if (filter_function(value.r) !== null) {
        not_found = false;
        callback(value.k, value.p, value.l);
      };
    })
