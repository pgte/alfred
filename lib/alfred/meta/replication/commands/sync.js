...
module.exports = function(master, stream) {
  ...
  temp_file.open(function(err, backlog_file) {
    ...
    key_maps.forEach(function(key_map_rec) {
      send({m: 'meta', command: 'attach_key_map', argument: [key_map_rec.name, key_map_rec.key_map.options]});
      var indexes = key_map_rec.key_map.indexes;
      for(index_name in indexes) {
        if (indexes.hasOwnProperty(index_name)) {
          (function(index_name) {
            var index = indexes[index_name];
            send({m: 'meta', command: 'add_index', argument: [key_map_rec.name, index_name, index.options, index.transform_function.toString()]})
          })(index_name);
        }
      };
    });
