...
Collection.prototype.write_at_pos = function(record, pos, callback) {
  if (record === null) {
    callback(new Error("collection.write_at_pos called with null record value"));
    return;
  }
  this.file.rawWrite("\n" + JSON.stringify(record) + "\n", pos, callback);
};
  ...
  this.file.written_size(function(err, size) {
    ...
    if (size <= 0) {
      ...
      (function keep_reading(file_pos) {
        ...
        self.file.try_read(file_pos, self.options.read_buffer_size, function(err, ret_buffer, bytesRead) {
          if (err) {
            callback(err);
            return;
          }
          ...
          last_10_reads.push({file_pos: file_pos, bytesRead: bytesRead, content:  content});
          if (last_10_reads.length > 10) {
            last_10_reads.splice(0, 1);
          }
          ...
          lines.forEach(function(one_line, index) {
            ...
            if (emit && line.length > 0) {
              last_10_lines.push(line);
              if (last_10_lines.length > 10) {
                last_10_lines.splice(0, 1);
              }
              var record;
              try {
                line_count += 2;
